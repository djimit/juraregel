export default function ArchitecturePage() {
  const layers = [
    { name: "Presentatie", color: "bg-blue-50 border-blue-500", components: ["Dashboard", "Assessment Workspace", "Regulatory Monitor", "Audit Console"] },
    { name: "API Gateway", color: "bg-green-50 border-green-500", components: ["REST (OpenAPI 3.1)", "GraphQL", "OAuth2 + mTLS", "Rate Limiting"] },
    { name: "Core Services", color: "bg-yellow-50 border-yellow-500", components: ["Template Service", "Assessment Engine", "Evidence Service", "Workflow Engine", "Compliance Scoring"] },
    { name: "AI / LLM", color: "bg-purple-50 border-purple-500", components: ["RAG Pipeline", "Knowledge Graph (NMF)", "Legal Reasoning", "Hallucination Detection"] },
    { name: "Data", color: "bg-red-50 border-red-500", components: ["PostgreSQL + RLS", "Qdrant (Vectors)", "Redis (Cache)", "Immutable Audit Log"] },
  ];
  ...
  const techStack = [
    ...
    { name: "Frontend", tech: "Next.js 14", desc: "React + Tailwind" },
          ...
          <div key={layer.name} className={`${layer.color} border-l-4 rounded-xl p-5`}>
            ...
            <div className="flex flex-wrap gap-2">
              {layer.components.map((c) => (
                <span key={c} className="bg-white px-4 py-2 rounded-lg text-sm font-medium text-gray-700 shadow-sm">{c}</span>
              ))}
            </div>
          </div>
        ))}
